...
function Save2(code){
	...
	if(slotclass != job && tmp.indexOf(" : no save") == -1){
		setAlertLayer("<br><br><br><br>Já existe uma build salva neste slot. Deseja sobrescrever?<br><br><br>"
			+ "<form><input type='button' value='Sim' onclick='Save3(code);'>&nbsp;&nbsp;&nbsp;"
			+ "<input type='button' value='Não' onclick='offOptionLayer();'></form>",0);
	}else{
		Save3(code);
	}
}
...
function Save3(code){
	...
	setAlertLayer("A build foi salva no Slot "+num+".",1);
}

function codeCopy(){
	tmp2 = "eduardoedson.github.io/"+job+".html?"+Save("scode");
	setAlertLayer("<br><br><br><br>Essa é a URL da sua build.<br><br><br>"
		+ "<form><input type='text' value='"
		+ tmp2 + "' onfocus='this.select();' onclick='this.select();' size=50 readonly></form>",0);
}
...
function decode(code,ver){
	...
	if(code.substring(0,2) != ver){
		setAlertLayer("Esse código é de uma versão diferente, use a versão atualizada do simulador.");
...
function loadType(mode){
	...
	if(mode == "cookie"){
		...
		if(slotdata[num].split(":")[0] != ""){
			...
			setAlertLayer("Carregando build do Slot "+num,1);
			document.Msg.chrName.value = unescape(name);
		}
	}if(mode == "url"){
		code = location.search.substring(1);
		setAlertLayer("Carregado da URL",1);
		document.Msg.scode.value = code;
	}if(mode == "tmp"){
		code = tmpcode;
	}
...
function Reset(){
	setAlertLayer("<br><br><br><br>Todos os pontos de habilidade serão resetados.<br><br><br><br>"
		+ "<form><input type='button' value='Ok' onclick='Clear();'>&nbsp;&nbsp;&nbsp;"
		+ "<input type='button' value='Cancelar' onclick='offOptionLayer();'></form>",0);
}
function DeleteSlot(){
	num = document.Msg.Data.options[document.Msg.Data.selectedIndex].value;
	setAlertLayer("<br><br><br><br>Deletar build do Slot "+num+"?<br><br><br><br>"
		+ "<form><input type='button' value='Sim' onclick='DeleteSlot2();'>&nbsp;&nbsp;&nbsp;"
		+ "<input type='button' value='Não' onclick='offOptionLayer();'></form>",0);
}
...
function DeleteSlot2(){
	...
	document.Msg.Data.options[document.Msg.Data.selectedIndex].style.backgroundColor = "#ffffff";
	setAlertLayer("Sua build foi deletada do Slot "+num+".",1);
}
